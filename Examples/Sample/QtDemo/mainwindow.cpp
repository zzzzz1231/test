    ...
    , ui(new Ui::MainWindow)
{
	...
	connect(this, SIGNAL(_wrap_update_menu()),				this, SLOT(_update_menu()), Qt::BlockingQueuedConnection);
	connect(this, SIGNAL(_wrap_update_parameter()),			this, SLOT(_update_parameter()), Qt::BlockingQueuedConnection);
...
void MainWindow::on_actionOperation_triggered()
{
	...
	auto pCam = m_R3_CTRL.at(m_currentSelection).m_this;
	if(pCam->getStatus() == SickCam::CAM_STATUS::CAM_IS_DISCONNECTED)
    {
        // --- Connect Ranger3
        auto err = pCam->connectCamera(&onCallback_LostHeartBeat, 1000, this);
        if (SickCam::CAM_STATUS::All_OK == err)
        {
            _log("Ranger3Studio: Connect " + m_currentSelection + "\n" + pCam->getDeviceInfoAsStructureString().c_str());
            _update_menu();
            _update_parameter();
        }
        else
        {
            _log("Ranger3Studio: [Error] Connect " + m_currentSelection + " failed, ErrorCode: " +
                 SickCam::CAM_STATUS_str(pCam->getStatus()).c_str());
        }
    }
    else if (pCam->getStatus() == SickCam::CAM_STATUS::CAM_IS_STOPPED ||
			 pCam->getStatus() == SickCam::CAM_STATUS::CAM_IS_CONNECTED)
    {
        // --- Disconnect Ranger3
...
void MainWindow::_update_parameter()
{
    ...
    if(m_R3_CTRL.count(m_currentSelection) == 1) {
        ...
        if (err != SickCam::CAM_STATUS::All_OK) {
			emit _log_emit(QString("Ranger3Studio:[Error] Get Parameters from device failed. ErrorCode: ") + SickCam::CAM_STATUS_str(err).c_str());
            _clear_parameter_tree_view();
            return;
        }
    }
...
void MainWindow::_update_menu()
{
	...
	{
		emit _log_emit("_update_menu, what?");
	}
...
void MainWindow::_viewer_welcome()
{
    try {
		...
		src = cv::imread(path + "/Examples/Sample/QtDemo/SICK.jpeg");
#endif
    ...
    catch(...)
    {
		...
		if (p != nullptr)
			path = std::string(p);
		_log(QString("Ranger3Studio: Welcome image(") + path.c_str() + "/Examples/Sample/QtDemo/SICK.jpeg) no found! ");
#endif
        return;
    }
}
...
onCallback_GetImage		(SickCam::ImgT * pImgD, void * any)
{
    ...
    switch (show_mode)
    {
        case SickCam::DataNames::RAN : // Range
            ...
            {
				QString fpath(pThis->m_save_image_path + std::to_string(pImgD->get_ID()).c_str());
				if(pImgD->SaveRawImagesToIconFile(fpath.toStdString(), true))
                        emit pThis->_log_emit("Ranger3Studio: Save raw image to " + fpath + " ok!");
                else
                        emit pThis->_log_emit("Ranger3Studio: [Error] Save raw image to " + fpath + ".dat|xml failed!");
            }
            break;
        ...
        case SickCam::DataNames::RAN_CAL :
            ...
            if(!pThis->m_save_image_path.isEmpty())
            {
				QString fpath(pThis->m_save_image_path + "_CAL_" + std::to_string(pImgD->get_ID()).c_str());
				if(pImgD->SaveCalibratedImagesToIconFile(fpath.toStdString(), true))
                        emit pThis->_log_emit("Ranger3Studio: Save calibrated image to " + fpath + " ok!");
                else
                        emit pThis->_log_emit("Ranger3Studio: [Error] Save calibrated image to " + fpath + ".dat|xml failed!");
            }
            break;
        ...
        case SickCam::DataNames::SEN : // sensor
            ...
            if(!pThis->m_save_image_path.isEmpty())
            {
				QString fpath(pThis->m_save_image_path + std::to_string(pImgD->get_ID()).c_str());
                if (pImgD->SaveRawImagesToIconFile(fpath.toStdString(), true))
                        emit pThis->_log_emit("Ranger3Studio: Save sensor image to " + fpath + " ok!");
                else
                        emit pThis->_log_emit("Ranger3Studio: [Error] Save sensor image to " + fpath + ".dat|xml failed!");
            }
            break;
        default:
            emit pThis->_log_emit(QString("[debug] default Viewer：") + SickCam::ImgT::DN2Str(show_mode).c_str());
            break;
    }
