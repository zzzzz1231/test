...
//	}
	...
	{
		//// ...existing code...
		//if (FactorX.type() != CV_64F) {
		//	cv::Mat tmp;
		//	FactorX.convertTo(tmp, CV_64F);
		//	tmp.copyTo(FactorX);
		//}
		//if (FactorY.type() != CV_64F) {
		//	cv::Mat tmp;
		//	FactorY.convertTo(tmp, CV_64F);
		//	tmp.copyTo(FactorY);
		//}
		//// 检查类型和尺寸
		//CV_Assert(FactorX.type() == CV_64F && FactorX.total() == 4);
		//CV_Assert(FactorY.type() == CV_64F && FactorY.total() == 4);
		//
		double xk1, xk2, xb1, xb2;
