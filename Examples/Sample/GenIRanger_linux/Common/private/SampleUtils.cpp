...
//std::string getPathToProducer() // windows
//{
...
//int32_t getNumericInput() // windows
//{
//  char input;
//  do
//  {
...
//bool parseDeviceOptionsHelper(int& argc, char* argv[], DeviceSelector& selector)  // windows
//{
//  bool failure = false;
//  int idx = 1;
//  while (idx < argc)
...
bool parseArgumentHelper(int argc,
                         ...
                         std::string& argumentValue) // windows
{
...
//uint32_t ip4FromString(const std::string& address) // windows
//{
//  uint32_t ipAddress;
...
uint32_t ip4FromString(const std::string& address) // ubuntu
{
    uint32_t ipAddress;
    //int result = InetPton(AF_INET, address.c_str(), &ipAddress);
...
//std::string stringFromIp4(uint32_t address) // windows
//{
...
std::string stringFromIp4(uint32_t address) // ubuntu
{
    char str[INET_ADDRSTRLEN];
    uint32_t addressNetworkByteOrder = htonl(address);
