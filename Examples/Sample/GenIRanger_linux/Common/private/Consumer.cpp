...
LocalUrl parseLocalUrl(const std::string& url)
{
  ...
  localUrl.address = std::strtoul(tokens[1].c_str(), &endptr_addr, 16);
  localUrl.length =
    static_cast<size_t>(std::strtoul(tokens[2].c_str(), &endptr_len, 16));
  return localUrl;
}
                                             ...
                                             std::string& deviceName)
{
  ...
  if (deviceId.empty())
  {
    std::cerr << "Device " << deviceName << " cannot be found." << std::endl;
    return GENTL_INVALID_HANDLE;
  }
  ...
  {
    std::cerr << "Connecting to device " << deviceName << std::endl
              << std::endl;
  }
                                           ...
                                           DeviceId& deviceId)
{
    if (deviceId.empty())
    {
        std::cerr << "Consumer: Device id string was empty." << std::endl;
        return GENTL_INVALID_HANDLE;
    }
    ...
    {
        std::cerr << "Consumer: Could not open device. " << getLastError() << std::endl;
        return GENTL_INVALID_HANDLE;
    }
    return deviceHandle;
}
    ...
    GenTL::DEV_HANDLE Consumer::openDeviceById(GenTL::IF_HANDLE interfaceHandle,
                                               DeviceId& deviceId, GenTL::GC_ERROR &status)
    {
        if (deviceId.empty())
        {
            std::cerr << "Consumer: Device id string was empty." << std::endl;
...
InterfaceList
Consumer::getInterfaces_is_changed(GenTL::TL_HANDLE hTL, bool & changed) {
    //bool8_t changed = false;
    // Timeout in milliseconds
    uint64_t timeout = 100;
    CR(mTl->TLUpdateInterfaceList(hTL, &changed, timeout));
    if (!changed)
    {
        std::cout << "[Consumer::getInterfaces_is_changed] unchanged" << std::endl;
        return InterfaceList();
    }
    std::cout << "[Consumer::getInterfaces_is_changed] changed" << std::endl;
...
DeviceList
Consumer::getDevices_is_changed(GenTL::IF_HANDLE ifHandle)
{
	...
	for (size_t i = 0; i < numDevices; ++i)
	{
		...
		CR(mTl->IFGetDeviceID(ifHandle, static_cast<uint32_t>(i), deviceId, &deviceIdSize));
		...
		// add model name check here
		{
			...
			char modelName[1024];
			size_t modelNameSize = sizeof(modelName);
			CR(mTl->IFGetDeviceInfo(ifHandle, deviceId, GenTL::DEVICE_INFO_MODEL, &dataType, modelName, &modelNameSize));
			...
			if (!is_SICK)
			{
				...
				CR(mTl->IFGetDeviceInfo(ifHandle, deviceId, GenTL::DEVICE_INFO_SERIAL_NUMBER, &dataType, sn, &snSize));
