...
/** An XmlElement can have either text contents or other XmlElements as
    children, but not both at the same time.
...
public:
  ...
  /** Validates that this XML tree is a super-set of the other given XML tree,
      meaning that children and attributes can be removed from this tree in
      order to form the other tree.

      Give two XML elements X and Y, X can be reduced to Y if all these
      criteria are meet:
      <ul>
      <li> X's name is the same as Y's name
      <li> Y's attributes is a subset of X's attributes.
      <li> Each of Y's children in order can be reduced to a child of X.
      </ul>
