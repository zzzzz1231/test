...
void SICK_CALLBACK
on_get_image_Demo_R3_callback_calibration_in_PC(SickCam::ImgT * pImg, void * any)
{
	std::cout << ((SickCam::Ranger3*)any)->getDeviceName() << ": Get Image : " << pImg->get_ID() << std::endl;
}

bool Demo_R3_callback_calibration_in_PC()
{
	...
	for (auto pCam : pCams)
	{
		...
		pCam->setCalibration_inPC("D:\\111.json", true, 2560, n_thread);
		// --- Start Ranger3
		CHECK(SickCam::CAM_STATUS::All_OK == pCam->startCamera(on_get_image_Demo_R3_callback_calibration_in_PC, pCam.get()));
	}
	...
	while (true) // 
	{
		...
		else
		{
			for (auto pCam : pCams)
			{
				...
				pCam->setCalibration_inPC("D:\\111.json", true, 2560, 2);
				// --- Start Ranger3
				CHECK(SickCam::CAM_STATUS::All_OK == pCam->startCamera(on_get_image_Demo_R3_callback_calibration_in_PC, pCam.get()));
			}
		}
...
{
	Demo_R3_callback_calibration_in_PC();
	return 0;
}
#endif
