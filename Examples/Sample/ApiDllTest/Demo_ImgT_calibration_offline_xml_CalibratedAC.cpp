...
bool SICK_GTEST_LIBRARY calibration_offline_xml_CalibratedAC(int argc, char* argv[])
{
	...
	CaliWrapper Cali;
	if (false == Cali.initCalibrationFilter(	"C:\\Users\\chenal\\Desktop\\20250331\\railleft0.xml",
												img.get_cols(),
												img.get_rows(),
												img.getR_aoiWidth(),
												img.getR_aoiHeight(),
	...
	if (false == Cali.calibrate(pD, pRange, pReflectance, nullptr, false)) // CalibratedAC
	{
	...
	imgOut.setCaliInfo(	pD->Width, 
						...
						pD->lower_bound_r, pD->upper_bound_r,1,0,1,0);
	
	imgOut.insertDataCopy(DataNames::RAN_X, (uint8_t*)pD->getData(SiCaliWrapper::RAN_X), 0);
	imgOut.insertDataCopy(DataNames::RAN_CAL, (uint8_t*)pD->getData(SiCaliWrapper::RAN_C), 0);
	//imgOut.insertDataCopy(DataNames::REF_CAL, (uint8_t*)pD->getData(SiCaliWrapper::REF_C), 0); // AC 模式下，Reflectance 无需标定
	imgOut.insertDataCopy(DataNames::REF_CAL, pReflectance, 0); // AC 模式下，Reflectance 无需标定
	...
	//	if (img.has(SickCam::DataNames::RAN_X))
...
{
	calibration_offline_xml_CalibratedAC();
	return 0;
}
#endif
