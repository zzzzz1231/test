...
ImageGrabber::connect()
{
	...
	try
	{
		...
		if (ec != Ecode::All_OK)
		{
			printf("Error: setCamera failed. para_csv_path=%s \n", m_pCam->getParametPath().c_str());

			//printf("Error: setCamera failed. para_csv_path=%s, calib_xml_path=%s \n"
			//	, m_pCam->getParametPath().c_str(), m_pCam->getCalibraPath().c_str()
			//);
...
{
	if (is_start_acquisition) return std::string();
...
{
	m_listImgTable.clear();
	m_listImgTable = std::list<SickCam::ImgT>();
