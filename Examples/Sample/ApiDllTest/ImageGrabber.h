...
*/
class ImageGrabber final
{
public:
	ImageGrabber();
	ImageGrabber(const std::shared_ptr<R3S> & _R3S, const std::string& _IP_MAC, const bool isIP);
	...
	*/
	bool setUserParameter(
						...
						const float missing_data_of_calibrated_image = MISSING_DATA);
