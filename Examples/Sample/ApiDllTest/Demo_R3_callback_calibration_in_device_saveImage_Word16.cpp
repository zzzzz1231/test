...
void SICK_CALLBACK
on_get_image_Demo_R3_callback_calibration_in_device_saveImage_Word16(SickCam::ImgT * pImg, void * any)
{
	auto id = pImg->get_ID();
	std::cout << "get: " << id << std::endl;
	if (id == 1)
		pImg->SaveCalibratedImagesToIconFile("D:/img1_3", true, false);
}

bool Demo_R3_callback_calibration_in_device_saveImage_Word16()
{
	...
	if (pR3S->isAvaliable())
	{
		...
		for (auto sub : devices)
		{
			...
			//  --- Init Ranger3
			pCam = std::make_shared<SickCam::Ranger3>(pR3S, sub.first, true);
			break;
		}
	}
	else
	...
	pCam->setCalibration_inDevice(2560, true, false);
	// --- Start Ranger3
	CHECK(SickCam::CAM_STATUS::All_OK == pCam->startCamera(on_get_image_Demo_R3_callback_calibration_in_device_saveImage_Word16, pCam.get()));

	// How to get height value?
	// 1. get Z scale from device
	//  float zOffset = atof(pCam->getParameter("Scan3dCoordinateOffset_Scan3dCoordinateSelector_CoordinateC_Scan3dExtractionSelector_Scan3dExtraction1"));
	//  float zScale  = atof(pCam->getParameter("Scan3dCoordinateScale_Scan3dCoordinateSelector_CoordinateC_Scan3dExtractionSelector_Scan3dExtraction1"));
	// 
	// 2. calculate height 
	//  float height = zScale * zValue16 + zOffset;
...
{
	Demo_R3_callback_calibration_in_device_saveImage_Word16();
	return 0;
}
#endif
