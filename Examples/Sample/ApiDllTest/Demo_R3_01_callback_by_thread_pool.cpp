...
void SICK_CALLBACK
on_callback_function_Demo_R3_01_callback_by_thread_pool(SickCam::ImgT * pImg, void * any)
{
	SickCam::Ranger3* pC = (SickCam::Ranger3*)any;
	std::cout << pC->get_DeviceIP()<< ": " << pImg->collectInfo() << std::endl;

	if (pImg->m_previousImageID + 1 != pImg->get_ID())
	{
		print_process_ng(pC->get_DeviceIP() + ": Image lost");
	}
}
...
bool SICK_GTEST_LIBRARY Demo_R3_01_callback_by_thread_pool()
{
	...
	for (int i = 0; i < ip.size(); ++i)
	{
		...
		pCams[i]->enableCallbackThreadsInPool(20, true);
	...
	for (int i = 0; i < ip.size(); ++i)
	{
		...
		pCams[i]->setCalibration_inDevice(3200);

		err = pCams[i]->startCamera(on_callback_function_Demo_R3_01_callback_by_thread_pool, pCams[i].get()); // pass variable to callback function
		if (SickCam::CAM_STATUS::All_OK != err)
		{
...
int main(int argc, char* argv[])
{
	...
	Demo_R3_01_callback_by_thread_pool();
	return 0;
}
#endif
