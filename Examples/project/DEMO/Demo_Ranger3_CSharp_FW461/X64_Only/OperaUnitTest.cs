...
namespace SampleApiDllCSharpWinForm
{
    public partial class frmMain : Form
    {
        ...
        private void _UnitTestThreadRun()
        {
            ...
            {
                var ok = MessageBox.Show("开始(OK) / 跳过(Cancel) _TestFourStatus2 ？ \n\nContinue?", "注意 Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                if (ok == DialogResult.OK)
            ...
            {
                var ok = MessageBox.Show("开始(OK) / 跳过(Cancel) _TestParamFile ？ \n\nContinue?", "注意 Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                if (ok == DialogResult.OK)
            ...
            {
                var ok = MessageBox.Show("开始(OK) / 跳过(Cancel) _TestParamSetting ？ \n\nContinue?", "注意 Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                if (ok == DialogResult.OK)
            ...
            {
                var ok = MessageBox.Show("开始(OK) / 跳过(Cancel) _TestSaveLoadImage ？ \n\nContinue?", "注意 Note", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                if (ok == DialogResult.OK)
        ...
        private void _TestParamSetting(ref bool res, ref int id, int total)
        {
            ...
            if (CAM_STATUS.All_OK != R3(m_ip).exportConfiguretionFile("tempFile_UnitTest_param.csv"))
            {
            ...
            if (CAM_STATUS.All_OK != R3(m_ip).importConfiguretionFile("tempFile_UnitTest_param.csv"))
            {
