...
namespace SampleApiDllCSharpWinForm
{
    public partial class frmMain : Form
    {
        ...
        public GenICam3D    m_r3s       = new GenICam3D("", GenICam3D.getPathToRanger3Producer(), false, true);
        ...
        public frmMain()
        {
            InitializeComponent();
            myTimer.Interval = 10;
            myTimer.Elapsed += onTimerGrabImage;
		...
		private void onTimerGrabImage(object sender, System.Timers.ElapsedEventArgs e)
        {
            // 响应停止请求
            if (m_reqStop == 0)
            {
        ...
        private void onThreadHeartBeat(string ip)
        {
			...
			if (!R3(ip).getHeartBeat())
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    {
                        // 继续尝试
						wFlog(i.ToString() + "/" + n.ToString() + "，继续尝试重新连接。");
                    }
