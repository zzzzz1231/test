...
void ParametersDlg::InitParameter(R3 * p, ImageGrabber *_pIG)
{
	m_pR3 = p;
	...
	GetDlgItem(ID_Test)->SetWindowTextW(_T("")); GetDlgItem(ID_Test)->EnableWindow(FALSE);
...
void ParametersDlg::InitForceIP(R3 * p, ImageGrabber *_pIG)
{
	m_pR3 = p;
	if (m_pR3 == nullptr) return;
	...
	GetDlgItem(ID_Test)->SetWindowTextW(_T(""));		GetDlgItem(ID_Test)->EnableWindow(FALSE);
	GetDlgItem(ID_Save)->SetWindowTextW(_T(""));		GetDlgItem(ID_Save)->EnableWindow(FALSE);
	GetDlgItem(ID_Import)->SetWindowTextW(_T(""));		GetDlgItem(ID_Import)->EnableWindow(FALSE);
	GetDlgItem(ID_Export)->SetWindowTextW(_T(""));		GetDlgItem(ID_Export)->EnableWindow(FALSE);
	GetDlgItem(ID_Calibrate)->SetWindowTextW(_T(""));	GetDlgItem(ID_Calibrate)->EnableWindow(FALSE);
	GetDlgItem(ID_ParamFile)->SetWindowTextW(_T(""));	GetDlgItem(ID_ParamFile)->EnableWindow(FALSE);
	GetDlgItem(ID_SaveImg)->SetWindowTextW(_T(""));		GetDlgItem(ID_SaveImg)->EnableWindow(FALSE);
...
void ParametersDlg::InitImageDataSetting(MFC_IMG * pI)
{
	...
	GetDlgItem(ID_ParamFile)->SetWindowTextW(_T("")); GetDlgItem(ID_ParamFile)->EnableWindow(FALSE);
	GetDlgItem(ID_Test)->SetWindowTextW(_T("")); GetDlgItem(ID_Test)->EnableWindow(FALSE);
	GetDlgItem(IDC_EDIT_CALI_PATH)->SetWindowTextW(_T("")); GetDlgItem(IDC_EDIT_CALI_PATH)->EnableWindow(FALSE);
	GetDlgItem(IDC_EDIT_PARA_PATH)->SetWindowTextW(_T("")); GetDlgItem(IDC_EDIT_PARA_PATH)->EnableWindow(FALSE);
	GetDlgItem(IDC_EDIT_SAVE_NAME)->SetWindowTextW(_T("")); GetDlgItem(IDC_EDIT_SAVE_NAME)->EnableWindow(FALSE);
	...
	m_data.m_vDesc = {
		...
		CString("[Read Only] The height of AOI in sensor image. Unit pixel."),
		CString("[Read Only] The scale of z in sensor. Unit pixel. If 16p, Default value is 0.0625(1/16)."),
		CString("[Read Only] The origin point of Z in sensor. Unit pixel. Defalut value is 0"),
		CString("[Read & Write] RectificationMethod."),
		CString("[Read Only] RectificationWidth. Default is 2560."),
	...
	m_data.m_vIsRW = { 0,0,0,1,0,0, 
		!m_pI->has(SICK_DN::RAN_CAL), !m_pI->has(SICK_DN::RAN_CAL), 0,
		0, 0, !m_pI->has(SICK_DN::RAN_CAL),
		0, !m_pI->has(SICK_DN::RAN_CAL), !m_pI->has(SICK_DN::RAN_CAL)
	};
...
void ParametersDlg::_InitUserParameter(bool is_forceIP_GUI)
{
	...
	size_t end__ = !is_forceIP_GUI ? m_user.m_vName.size() : 9;
	for (size_t i = begin; i<end__; ++i)
	{
...
void ParametersDlg::_UpdateParameterSettings()
{
	...
	for (int i = 0; i<m_propertiesUser.size(); ++i)
	{
		...
		if (is_updated)
		{
			...
			std::stringstream ss; ss << ", Execution Code = " << SickCam::CAM_STATUS_str(e);
...
void ParametersDlg::OnBnClickedCalibrate()
{
	...
	{
		CFileDialog dlg(TRUE, NULL, NULL, OFN_HIDEREADONLY, _T("XML Files(*.xml)||JSON Files(*.json)||"));
		if (IDOK != dlg.DoModal())
			return;
		...
		if (strPath.Mid(strPath.GetLength() - 3, 3) == CString("xml") 
			|| strPath.Mid(strPath.GetLength() - 4, 4) == CString("json"))
		{
...
void ParametersDlg::OnBnClickedImport()
{
	...
	if (strPath.Mid(strPath.GetLength() - 3, 3) == CString("csv"))
	{
		...
		ss << "Import (Load from < " << path.c_str() << " > ) parameter file to device. Execution Code = " << SickCam::CAM_STATUS_str(e);
		// update window
...
void ParametersDlg::OnBnClickedExport()
{
	...
	std::stringstream ss;
	ss << "Export (Save to < "<< path.c_str() << " >) parameter file from device. Execution Code = " << SickCam::CAM_STATUS_str(e);
