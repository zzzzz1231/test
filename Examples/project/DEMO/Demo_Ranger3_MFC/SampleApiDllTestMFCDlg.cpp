...
MFC_IMG::MFC_IMG(const SICK_DATA & _data, const CString _path=CString(""))
{
	...
	if (!m_pImg->isEmpty())
	{
		...
		m_caliPath = CString("");
		m_savePath = CString("");
		m_aoiHeigh = CString(std::to_string(_data.getR_aoiHeight()).c_str());
		m_scaleZ   = CString((_data.getR_RangeAxis() == RangeAxis::STANDARD ? "0.0625":"-0.0625"));
		m_originZ  = CString(std::to_string(_data.getR_aoiOffsetY()).c_str());
		m_RectMeth = CString("0 | Top");
		m_RectWidt = CString(std::to_string(_data.getR_cols()).c_str());
...
void CSampleApiDllTestMFCDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_CAM1_RANGE, m_pictureControl);
}
...
BOOL CSampleApiDllTestMFCDlg::OnInitDialog()
{
	...
	m_pR3S = std::make_shared<R3S>(logPath, R3S::getPathToRanger3Producer());

	if (m_pR3S->isCtiFound())	{
		log(CString("CTI_FOUND!"));
	}
	else
	{
	...
	if (!m_pR3S->isAvaliable())
	{
		...
		SetWindowTextW(CString("SampleApiDllTestMFCDlg - NO_Device_FOUND!"));
		log(CString("NO_Device_FOUND!"));
	}
...
#ifndef UNIT_TEST
	GetDlgItem(IDC_BUTTON_SelfTest)->SetWindowTextW(_T(""));
	GetDlgItem(IDC_BUTTON_SelfTest)->EnableWindow(FALSE);
#endif
...
void CSampleApiDllTestMFCDlg::saveOnlineImageToPath(const CString path)
{
	...
	if (!m_currentR3Image.isEmpty())
	{
		...
		//img.SaveToIconFile(spath, img.getAvalibleDataNamesRaw());
		//img.SaveToIconFile(spath+"_cal", img.getAvalibleDataNamesCal());
		ss << ". \r\n OK";
	}
	else
...
{
	SICK_DATA img;
	if (img.loadFromIconFile(std::string(CT2A(path)), 1.0))
	{
...
void CSampleApiDllTestMFCDlg::_showMatImgToWnd(CWnd* pictureWnd, const cv::Mat & _disimg)
{
	cv::Mat disimg;
	cv::Mat background = cv::Mat::zeros(480, 640, CV_8UC1);
	if (_disimg.empty())
	{
		disimg = cv::Mat::zeros(480, 640, CV_8UC1);
		cv::putText(disimg, "EMPTY", cv::Point(320, 240), cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 2);
	}

	CRect drect;
	pictureWnd->GetClientRect(drect);

	disimg = _disimg.clone();
	m_IFCV->removeNAN(disimg);
	cv::normalize(disimg, disimg, 0.0, 255.0, cv::NORM_MINMAX);
	disimg.convertTo(disimg, CV_8UC1);

	cv::Size dstSize;
	bool isHigher = (double(disimg.rows) / disimg.cols) > (double(drect.Height()) / drect.Width()); // 480/640 = 0.75
	if (!isHigher)
		dstSize = cv::Size(drect.Width(), drect.Width() * (double(disimg.rows) / disimg.cols));
	else
		dstSize = cv::Size(drect.Height() * (double(disimg.cols) / disimg.rows), drect.Height());
	...
	// draw background
	{
		...
		bitMapinfo->bmiHeader.biHeight = -background.rows;
		bitMapinfo->bmiHeader.biWidth = background.cols;
		bitMapinfo->bmiHeader.biBitCount = background.channels() * 8;
	...
	const size_t rectificationWidth,	float missingDataValue,	const bool _doRectify, CString currentItemName)
{
	...
	if (false == pCalibrationWrapper->initCalibrationFilter(
		std::string(CT2A(pathXML)), 
		img.getR_cols(),
		img.getR_rows(),
		aoiHeight, 
		img.get_AoiWidth(),
		img.get_SICK_DATA().getRange_info().xml_scale_x,
		img.get_SICK_DATA().getRange_info().xml_origin_x,
		scaleZ, 
		originZ, 
		16, // Scatter 图像一个值的默认字节数
		method,
		rectificationWidth,
		missingDataValue, 
		_doRectify,
	...
	if (img.has(SICK_DN::SCA_8))
	{
		size_t sizeT(img.getR_rows()* img.getR_cols() * sizeof(uint8_t));
		p3 = new uint8_t[sizeT];
		memcpy(p2, (uint8_t*)img.get_SICK_DATA().getData(SICK_DN::SCA_8), sizeT);
	}
	else
	{
		log(CString("SICK_DN::SCA_8 is empty! Continue ..."));
	}

	if (img.has(SICK_DN::SCA_16))
	{
		size_t sizeT(img.getR_rows()* img.getR_cols() * sizeof(uint16_t));
		p3 = new uint8_t[sizeT];
		memcpy(p2, (uint8_t*)img.get_SICK_DATA().getData(SICK_DN::SCA_16), sizeT);
	}
	else
	{
		log(CString("SICK_DN::SCA_8 is empty! Continue ..."));
	...
	// calibrate
	auto cpData = std::make_shared<SiCaliWrapper::calibratedData>(img.getR_cols(), img.getR_rows());
	pCalibrationWrapper->calibrate(cpData, p1, p2, p3, true);
	...
	timgT.setCaliInfo(
		...
		cpData->offsetX,0.0,0.0, cpData->scaleX, img.get_scaleY(),1.0,
...
BOOL CSampleApiDllTestMFCDlg::_SaveAsSICKDAT(MFC_IMG mfc, CString _path, SICK_DN _nd)
{
	...
	{
		ss << "Save OK!";	log(CString(ss.str().c_str()));
		return TRUE;
	}
	ss << "Save Failed!";	log(CString(ss.str().c_str()));
	return FALSE;
...
void CSampleApiDllTestMFCDlg::OnScanDevice()
{
	...
	if (!m_Name_Ranger3s.empty())
	{
		for (auto sub : m_Name_Ranger3s)
		{
			if (sub.second->isStartAcquition())
			{
				...
				while (sub.second->isStartAcquition()) Sleep(10);
			}

			if (sub.second->isConnected())
			{
				sub.second->requireDisconnect();
				while (sub.second->isConnected()) Sleep(10);
			}
		}
	...
	std::stringstream ss;
	ss << "Scan device done! Execution Code (0 is ok) = " << SickCam::CAM_STATUS_str(e);
	...
	if (devices.empty())
	{
		...
		{
			sub.second->requireDisconnect();
			while (!sub.second->isDisconnected()) Sleep(10);
			sub.second->clearData();
		}

		m_Name_Ranger3s.clear();
		for (auto sub : m_Name_Thread)
		{
			delete sub.second;
		}
		m_Name_Thread.clear();

		for (auto sub : m_Name_Parameters)
		{
			delete sub.second;
		}
	...
	{
		auto pIG = std::make_shared<ImageGrabber>(m_pR3S, sub.second->getIp(), true);
		if (!sub.first.empty())
		{
...
void CSampleApiDllTestMFCDlg::OnConnectRanger3(CString R3Name)
{
	if (m_Name_Ranger3s.count(R3Name) == 1)
	{
		...
		if (ok)
		{
			...
			std::thread* thread_pImgGabber = new std::thread(&ImageGrabber::run, p);
...
void CSampleApiDllTestMFCDlg::OnStopRanger3()
{
	...
	if ((1000 <= curSelTab && curSelTab < 2000) | m_isTest) // device items: 1000-2000
	{
		if (m_Name_Ranger3s.count(m_ID_R3Name[curSelTab]) == 1)
		{
			...
			if (p->isConnected() && p->isStartAcquition())
			{
				//OutputDebugString(_T("requireStopAcquition"));
				p->requireStopAcquition();
				while (p->isStartAcquition()) Sleep(10);

				//OutputDebugString(_T("StopTimer()"));
				...
				log((!true ? CString("## Failed ## ") : CString("")) +
